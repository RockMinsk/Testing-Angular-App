...
describe('contact list', () => {
    ...
    describe('read contact list', () => {
        it('should find the new contact', () => {
            const tableRow = contactList.findContact('Mr. Newton').get(0);
            const tableData = tableRow.all(by.tagName('td'));
        ...
        it('should get the contact names so we can print them to console', () => {
            const contactNames = contactList.getContactNames();
            const expectedNames = 'Adrian Directive, Rusty Component, Jeff Pipe, Craig Service, Mr. Newton';
